  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const heroLines = gsap.utils.toArray<HTMLElement>(
        hero.querySelectorAll(".hero-line"),
      );

      if (prefersReducedMotion) {
        if (mapaEl) gsap.set(mapaEl, { opacity: 1, scale: 1 });
        if (heroLines.length) gsap.set(heroLines, { y: 0, opacity: 1 });
      } else if (mapaEl && heroLines.length) {
        gsap.set(mapaEl, { opacity: 0 });
        gsap.set(heroLines, { y: 56, opacity: 0 });

        const heroIntroTl = gsap.timeline();
        heroIntroTl.to(mapaEl, {
          opacity: 1,
          duration: 1.55,
          ease: "power2.out",
          onComplete: () => {
        ...
        const fallbackLines = hero.querySelectorAll(".hero-line");
  ...
  return (
    <>
      ...
      >
        <a
          href="#contenido"
          className="fixed right-4 top-4 z-[100] -translate-y-20 rounded bg-brand-gold px-4 py-2 font-body text-sm font-medium text-brand-bg opacity-0 transition focus:translate-y-0 focus:opacity-100 focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-brand-cream"
        >
          Ir al contenido
        </a>
        ...
        <main id="contenido">
          <section
            id="inicio-republica"
            ref={heroRef}
            className="relative flex min-h-[100dvh] flex-col items-center justify-center overflow-hidden px-6 pt-12 pb-28 md:pt-14"
          >
          ...
          >
            <div className="mx-auto grid max-w-6xl items-center gap-12 md:grid-cols-2 md:gap-16">
              <div className="order-2 md:order-1">
                ...
                <p
                  id="historia-texto"
                  className="mt-6 font-body text-lg leading-relaxed text-slate-700 md:text-xl"
                  data-reveal
                >
                  {HISTORIA_COPIA}
                </p>
